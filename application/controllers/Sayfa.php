...
class Sayfa extends MY_Controller {
	...
	public function __construct()
	{
		...
		parent::__construct();
		$this->klasor="genel_v";
        $this->model="sayfa_detay_model";

		$this->load->model("dinamik/sayfa_model");
		$this->load->model("dinamik/sayfa_detay_model");

        $this->baslik="Sayfalar";
        $this->stamp_id=6;
        $this->yonlendirme="sayfa";
	...
	public function dilekle($id)
	{
		...
		$eksikdil=$this->sayfa_detay_model->cek(
			array(
				'grup_id'=>$id,
				'dil'=>1
			)
        ...
        $gruptakiler=$this->sayfa_detay_model->tumu(
            array(
                'grup_id'=>$id,
	...
	public function duzenle($id)
	{
		...
		$viewData =new stdClass() ;
		$item=$this->sayfa_detay_model->cek(
			array(
				'id'=>$id,
	...
	public function medya($id)
	{
		...
		$viewData =new stdClass() ;
		$viewData->item=$item=$this->sayfa_model->cek(
			array(
				'id'=>$id
			)
		);
	...
	public function kaydet() // Yeni ürün ekleme
	{
		...
		if($form_kontrol) {
			...
			$kayit = $this->sayfa_model->vt_kayit(
				array(
			...
			if($kayit) {
				...
				foreach ($diller as $dil) {
					$kaydet = $this->sayfa_detay_model->vt_kayit(
						array(
							"title" => $this->input->post("title"),
	...
	public function guncelle($id)
	{
        ...
        $update = $this->sayfa_detay_model->vt_guncelle(
            array(
                'id'=>$id
            ),
    ...
    public function grupguncelle($id)
    {
        ...
        $update = $this->sayfa_model->vt_guncelle(
            array(
                'id'=>$id
            ),
	...
	public function sil($id)
	{
		...
		$tumusil = $this->sayfa_detay_model->vt_sil(
			array(
				'grup_id'=>$bul->grup_id
			)
			);
